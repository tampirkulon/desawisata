  ...
  container.innerHTML = `
    ...
    <main class="pt-8 pb-16 px-4 md:px-16 max-w-container-max mx-auto w-full flex-grow flex flex-col justify-center">
      <div class="mb-12 text-center md:text-left">
        ...
        <p class="font-body-md text-base text-on-surface-variant max-w-2xl">Rencanakan kunjungan Anda atau hubungi kami untuk informasi lebih lanjut. Kami siap menyambut Anda di ${namaDesa}.</p>
      </div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div class="lg:col-span-7 bg-surface-container-lowest rounded-2xl p-6 md:p-10 shadow-level-1 border border-outline-variant/30">
          ...
          <form id="reservasi-form" class="space-y-6">
            ...
            <button class="w-full md:w-auto bg-primary text-white px-8 py-3.5 rounded-full font-bold text-sm hover:bg-primary-container transition-all shadow-level-1 cursor-pointer flex items-center justify-center gap-2" type="submit">
        ...
        <div class="lg:col-span-5 flex flex-col gap-6">
          <div class="bg-primary-container text-on-primary rounded-2xl p-8 shadow-level-1">
            <h3 class="font-display-lg text-xl font-bold mb-6 text-tertiary-fixed">Informasi Kontak Pengelola</h3>
            <ul class="space-y-6 list-none p-0 m-0">
              <li class="flex items-start gap-4">
                <span class="material-symbols-outlined text-tertiary-fixed text-2xl shrink-0 mt-0.5">location_on</span>
                <div>
                  <span class="block font-label-caps text-xs text-on-primary-container font-bold uppercase mb-1">ALAMAT DESA</span>
                  <span class="font-body-md text-sm text-white/90 leading-relaxed">${profil.alamat || 'Jl. Raya Candimulyo No. 12, Tampirkulon, Magelang, Jawa Tengah 56191'}</span>
                </div>
              </li>
              <li class="flex items-center gap-4">
                <span class="material-symbols-outlined text-tertiary-fixed text-2xl shrink-0">call</span>
                <div>
              ...
              <li class="flex items-center gap-4">
                <span class="material-symbols-outlined text-tertiary-fixed text-2xl shrink-0">mail</span>
                <div>
            ...
            ${cleanWhatsapp ? `
              <div class="mt-6 pt-6 border-t border-white/20">
                <a href="https://wa.me/${cleanWhatsapp}?text=Halo%20Pengelola%20${encodeURIComponent(namaDesa)},%20saya%20ingin%20bertanya%20mengenai%20kunjungan%20wisata." target="_blank" rel="noopener noreferrer" class="w-full py-3 px-4 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-xs transition-all shadow-sm flex items-center justify-center gap-2">
            ...
            <div class="mt-6 pt-6 border-t border-white/20">
              ...
              <div class="flex justify-between items-center text-sm">
                <span>Pelayanan Wisata</span>
                <span class="font-bold text-tertiary-fixed">${profil.jam_operasional || 'Senin - Minggu: 08:00 - 17:00 WIB'}</span>
              </div>
            </div>
          </div>
      ...
      form.addEventListener('submit', async (e) => {
        ...
        if (isSupabaseConfigured() && supabase) {
          try {
            ...
            alertBox.innerHTML = `✅ Reservasi berhasil dikirim! Tim pengelola ${namaDesa} akan segera menghubungi WhatsApp Anda.`;
          ...
          alertBox.className = 'mb-6 p-4 rounded-xl text-sm font-semibold bg-emerald-100 text-emerald-800 border border-emerald-300';
          alertBox.innerHTML = `✅ Reservasi berhasil disimpan (Mode Demo)! Pengelola ${namaDesa} akan segera menghubungi Anda.`;
          alertBox.classList.remove('hidden');
          form.reset();
        }
      });
