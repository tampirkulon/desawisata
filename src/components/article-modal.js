...
export const openArticleModal = (article) => {
  ...
  modal.innerHTML = `
    <div class="bg-surface rounded-2xl max-w-3xl w-full max-h-[90vh] overflow-y-auto shadow-level-2 border border-outline-variant/30 flex flex-col relative animate-scale-up">
      <!-- Close Button -->
      <button id="close-article-modal" class="absolute top-4 right-4 z-10 w-10 h-10 rounded-full bg-surface-container-high/80 hover:bg-surface-container-highest text-on-surface flex items-center justify-center transition-all cursor-pointer shadow-md">
