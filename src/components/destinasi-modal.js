...
export const openDestinasiModal = (destinasi) => {
  ...
  modal.innerHTML = `
    <div class="bg-surface rounded-2xl max-w-3xl w-full max-h-[90vh] overflow-y-auto shadow-level-2 border border-outline-variant/30 flex flex-col relative animate-scale-up">
      <!-- Close Button -->
      <button id="close-destinasi-modal" class="absolute top-4 right-4 z-10 w-10 h-10 rounded-full bg-black/50 hover:bg-black/80 text-white flex items-center justify-center transition-all cursor-pointer shadow-md">
      ...
      ${images.length > 1 ? `
        <div class="flex gap-2 p-3 bg-surface-container-low overflow-x-auto border-b border-outline-variant/20">
          ${images.map((img, idx) => `
            <img src="${img}" class="w-16 h-12 object-cover rounded-lg border-2 border-transparent hover:border-primary cursor-pointer modal-thumb transition-all ${idx === 0 ? 'border-primary' : ''}" data-src="${img}" />
          `).join('')}
        </div>
      ` : ''}
      ...
      <div class="p-6 md:p-8 space-y-6">
        <div>
          <h2 class="font-display-lg text-2xl md:text-3xl font-bold text-primary mb-3">${destinasi.nama}</h2>
          <div class="flex items-center gap-4 text-xs text-on-surface-variant flex-wrap">
            <span class="flex items-center gap-1 font-semibold"><span class="material-symbols-outlined text-sm text-primary">location_on</span> ${destinasi.lokasi || 'Tampirkulon'}</span>
            ...
            <span class="flex items-center gap-1 font-bold text-emerald-800 bg-emerald-100 px-3 py-1 rounded-full"><span class="material-symbols-outlined text-sm">confirmation_number</span> ${destinasi.harga_tiket || 'Gratis'}</span>
          </div>
        </div>

        <div class="border-t border-outline-variant/20 pt-4">
          <h3 class="font-display-lg text-sm font-bold text-slate-800 uppercase tracking-wider mb-2">Tentang Destinasi</h3>
          <p class="font-body-md text-sm text-on-surface-variant leading-relaxed">${destinasi.deskripsi || 'Destinasi wisata unggulan di Desa Tampirkulon.'}</p>
        </div>

        <!-- CTA Action -->
        <div class="pt-4 border-t border-outline-variant/30 flex items-center justify-between flex-wrap gap-4">
          <div>
            <span class="text-xs text-on-surface-variant block font-medium">Tertarik berkunjung ke sini?</span>
